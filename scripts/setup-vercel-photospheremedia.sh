#!/usr/bin/env bash
# PhotoSphere Vercel setup (import on vercel.com may already be done).
# See docs/VERCEL_MIGRATION.md for post-import checklist.
#
# Prereqs:
...
Steps (already imported on vercel.com):
  1. vercel login  (PhotoSphere team GitHub — NOT personal alt accounts)
  2. Confirm dashboard build settings match vercel.json (see docs/VERCEL_MIGRATION.md)
  3. pnpm vercel:link && pnpm vercel:env-sync && pnpm vercel:secrets
  4. Production branch: master

Monorepo (vercel.json at repo root):
  install: pnpm install --frozen-lockfile
  build:   pnpm --filter @crypto-pay/portal build
  output:  (empty — Next.js on Vercel; do not set apps/portal/.next)
